...
:::tip[Solo para las personas curiosas]
Podemos construir la aplicación con máquinas de estado usando 
herencia. Te muestro cómo se hace en la sección oculta a continuación.
:::

<details>
<summary>Implementación usando herencia</summary>
El editor de micropython del micro:bit soporta aplicaciones 
con varios archivos. Puedes acceder a esta característica en 
el menú Projects. Allí crearás un par de archivos llamados 
main.py y fsm.py. En fsm.py tendremos la implementación 
de la clase base para las máquinas de estados y en main.py
la implementación específica de la aplicación en cuestión.
...
class Game(FSMTask):
    def __init__(self):
        ...
        self.timeInPacman = 1000
        self.timeInGhost = 2000
        self.myTimer = self.add_timer("Timeout",self.timeInHeart)
        self.transition_to(self.estado_waitInHeart)
    ...
    def estado_waitInHeart(self, ev):
        if ev == ENTRY:
            ...
            self.myTimer.start(self.timeInHeart)
        if ev == "Timeout":
            self.transition_to(self.estado_waitInPacman)
        if ev == "A":
            self.transition_to(self.estado_waitInGhost)
...
while True:
    if button_a.was_pressed(): game.post_event("A")
    game.update()
    utime.sleep_ms(20)
```
